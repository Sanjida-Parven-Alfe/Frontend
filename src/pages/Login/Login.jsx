...
const Login = () => {
  ...
  // 1. Regular Login (No need to save to DB, they already exist)
  const onSubmit = (data) => {
  ...
  // 2. Google Login (Must Save to DB for new Google Users)
    ...
    <div className="flex min-h-screen bg-[#0f172a] font-sans -mt-24">
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center bg-cover bg-center relative" style={{ backgroundImage: `url('https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?q=80&w=2000&auto=format&fit=crop')` }}>
        ...
        <div className="relative z-10 text-center text-white p-12">
          ...
          <p className="text-xl font-light max-w-lg mx-auto leading-relaxed text-gray-200">Your gateway to extraordinary interior designs.</p>
        </div>
      </div>

      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 relative z-0" style={{ backgroundColor: "#0f172a", backgroundImage: `radial-gradient(at 0% 0%, hsla(353,96%,66%,0.2) 0px, transparent 50%), radial-gradient(at 50% 0%, hsla(176,61%,55%,0.2) 0px, transparent 50%), radial-gradient(at 100% 0%, hsla(49,100%,71%,0.2) 0px, transparent 50%)` }}>
        <div className="w-full max-w-[450px] mt-20 lg:mt-0">
          ...
          <button onClick={handleGoogleLogin} className="btn btn-outline w-full h-12 border-gray-600 hover:bg-white hover:text-black text-white font-medium normal-case flex items-center justify-center gap-3 transition-all duration-200 rounded-lg mb-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="form-control w-full">
              <label className="label pb-1.5"><span className="label-text font-semibold text-gray-300">Email Address</span></label>
            ...
            <div className="form-control w-full">
              <div className="flex justify-between items-center pb-1.5">
                <label className="label p-0"><span className="label-text font-semibold text-gray-300">Password</span></label>
                <a href="#" className="text-xs font-medium text-teal-400 hover:text-teal-300 hover:underline">Forgot password?</a>
              </div>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} placeholder="Enter your password" className={`input input-bordered pl-2 w-full h-12 bg-white/5 border-white/10 focus:border-teal-400 text-white placeholder-gray-500 transition-all rounded-lg pr-12 ${errors.password ? "input-error bg-red-900/10" : ""}`} {...register("password", { required: "Password is required" })} />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-4 flex items-center z-10 text-gray-400 hover:text-white cursor-pointer">
          ...
          <p className="text-center text-sm text-gray-400 mt-6">
            Don't have an account? <Link to="/signup" className="font-bold text-white hover:text-teal-400 hover:underline transition-all ml-1">Create free account</Link>
          </p>
        </div>
      </div>
    </div>
