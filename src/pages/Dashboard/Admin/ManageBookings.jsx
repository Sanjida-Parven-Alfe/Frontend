...
const ManageBookings = () => {
    ...
    const handleAssignDecorator = async (id) => {
        ...
        if (decoratorName) {
            ...
            axiosSecure.patch(`/bookings/${id}`, { decoratorName })
                .then(res => {
                    if (res.data.modifiedCount > 0) {
                        refetch();
                        Swal.fire('Assigned!', `Decorator ${decoratorName} has been assigned.`, 'success');
                    }
                })
        }
    };
    ...
    return (
        <div className="w-full">
            <h2 className="text-3xl font-bold mb-8 text-white">Manage <span className="text-brand-yellow">Bookings</span> ({bookings.length})</h2>
                        ...
                        {bookings.map((booking) => (
                            <tr key={booking._id} className="border-b border-white/5 hover:bg-white/5">
                                <td className="font-bold">
                                    {booking.userName} <br/>
                                    <span className="text-xs font-normal text-gray-500">{booking.userEmail}</span>
                                </td>
                                ...
                                <td>
                                    <span className={`badge ${booking.status === 'confirmed' ? 'badge-success' : 'badge-warning'} badge-outline text-xs capitalize`}>
