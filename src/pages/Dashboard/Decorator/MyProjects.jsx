...
const MyProjects = () => {
  ...
  const { data: projects = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/decorator/bookings/${user?.displayName}`
      );
      return res.data;
    },
  });
  ...
  const handleStatusUpdate = (id, newStatus) => {
    axiosSecure
      .patch(`/decorator/status/${id}`, { status: newStatus })
      .then((res) => {
      ...
      {projects.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {projects.map((project) => (
            <div
              key={project._id}
              className="glass-card p-6 rounded-2xl border border-white/10 bg-[#1e293b] hover:border-brand-teal/50 transition-all duration-300"
            >
              ...
              <div className="form-control w-full">
                ...
                <select
                  className="select select-bordered pl-2 w-full bg-[#0b1120] border-gray-600 text-white focus:border-brand-teal"
