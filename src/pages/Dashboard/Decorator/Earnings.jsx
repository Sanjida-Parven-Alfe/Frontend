...
import { FaWallet, FaMoneyBillWave, FaHistory, FaCheckCircle, FaTimesCircle } from 'react-icons/fa';
...
const Earnings = () => {
    ...
    const paidProjects = bookings.filter(item => item.paymentStatus === 'paid').length;
        ...
        <div className="w-full p-4 md:p-8 bg-[#0f172a] min-h-full text-white">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="stat glass-card bg-gradient-to-r from-green-900 to-green-600 rounded-2xl text-white border-none shadow-xl">
                    ...
                    <div className="stat-title text-green-100 font-medium">Total Received</div>
                    <div className="stat-value text-4xl font-bold mt-2">৳{totalEarned}</div>
                    <div className="stat-desc text-green-200 mt-1">{paidProjects} Projects Paid</div>
                </div>
                ...
                <div className="stat glass-card bg-gradient-to-r from-orange-900 to-orange-600 rounded-2xl text-white border-none shadow-xl">
                    ...
                    <div className="stat-title text-orange-100 font-medium">Pending Payments</div>
                    <div className="stat-value text-4xl font-bold mt-2">৳{pendingPayment}</div>
                ...
                <div className="stat glass-card bg-gradient-to-r from-blue-900 to-blue-600 rounded-2xl text-white border-none shadow-xl">
                    ...
                    <div className="stat-title text-blue-100 font-medium">Total Projects</div>
                    <div className="stat-value text-4xl font-bold mt-2">{bookings.length}</div>
                            ...
                            <tbody className="divide-y divide-white/5">
                                {bookings.map(item => (
                                    <tr key={item._id} className="hover:bg-white/5 transition-colors">
                                        <td className="p-4 font-bold text-white">{item.serviceName}</td>
                                        <td className="p-4 text-sm">{item.userName}</td>
                                        <td className="p-4 text-sm text-gray-400">{item.date}</td>
                                        <td className="p-4 font-bold text-brand-yellow">৳{item.price}</td>
                                        <td className="p-4">
