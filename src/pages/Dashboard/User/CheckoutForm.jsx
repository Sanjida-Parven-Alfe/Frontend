...
const CheckoutForm = ({ booking, price }) => {
  ...
  useEffect(() => {
    if (price > 0) {
      axios
        .post(
          "https://backend-delta-sable-65.vercel.app/create-payment-intent",
          { price },
          {
            headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    if (paymentIntent.status === "succeeded") {
      ...
      axios
        .post("https://backend-delta-sable-65.vercel.app/payments", payment, {
          headers: {
            authorization: `Bearer ${localStorage.getItem("access-token")}`,
          },
        })
  ...
  return (
    <>
      <form
        ...
        className="w-full md:w-2/3 mx-auto mt-8 p-6 bg-white/5 rounded-xl border border-white/10"
      >
        <div className="mb-4">
      ...
      {cardError && (
        <p className="text-red-500 mt-4 text-center text-sm">{cardError}</p>
      )}
      {transactionId && (
        <p className="text-green-400 mt-4 text-center text-sm">
