...
const PaymentHistory = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      axios
        .get(
          `https://backend-delta-sable-65.vercel.app/payments/${user.email}`,
          {
            headers: {
  ...
  return (
    <div className="w-full">
      <h2 className="text-3xl font-bold mb-8 text-white">
        Payment <span className="text-brand-teal">History</span>
      </h2>
        ...
        <table className="table w-full text-gray-300">
          ...
          <tbody>
            {payments.map((payment, index) => (
              ...
              >
                <th>{index + 1}</th>
                <td className="font-mono text-xs text-brand-yellow">
